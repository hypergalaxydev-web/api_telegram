...
class SystemPreferenceForm extends TStandardForm
{
    ...
    function __construct()
    {
        ...
        $row = $this->form->addFields( [new TLabel(_t('Enable Google reCAPTCHA'), null, null, null, '100%'), $google_recaptcha] );
        $row->layout = ['col-sm-6'];
        ...
        $row = $this->form->addFields( [new TLabel(_t('Enable 2FA by email'), null, null, null, '100%'), $e2fa_by_email], [new TLabel(_t('Enable 2FA by Google Authenticator'), null, null, null, '100%'), $e2fa_by_google_auth] );
        $row->layout = ['col-sm-6', 'col-sm-6'];
